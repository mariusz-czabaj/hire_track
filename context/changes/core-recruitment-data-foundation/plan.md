...
**Contract**: Inserts, in order: `security_groups` rows named `HR/Rekruter`, `Hiring Manager`, `Administrator`; `group_operations` rows granting HR both `recruitment.*`/`candidate.*` operations, Hiring Manager only the two `*.read` operations, Administrator only `group.manage`; three rows directly into `auth.users` — one per group (standard Supabase local-seed pattern — encrypted password via `extensions.crypt`) plus their `group_memberships` rows into `HR/Rekruter`, `Hiring Manager`, and `Administrator` respectively (the Hiring Manager and Administrator accounts exist specifically so `rls_verification.sql`'s assertions have a read-only user and a zero-visibility user to test against, not just the HR/Rekruter admin-equivalent account); one `recruitments` row + its `recruitment_security_groups` links to both `HR/Rekruter` and `Hiring Manager` (not `Administrator` — that's the point of the isolation assertion); 4-5 `candidates` rows linked via `candidate_recruitments` across several of the 6 default `kanban_stages`, each with one `candidate_recruitment_status_history` row recording the initial add.
