...
| accessibility          | `@axe-core/playwright`                          | ^4     | `eslint-plugin-jsx-a11y` covers static rules; `tests/e2e/accessibility.spec.ts` runs an axe (`wcag2a`/`wcag2aa`) scan of sign-in, recruitments list, kanban board, candidate profile and admin groups, in both light/dark themes. Axe catches roughly 30-40% of WCAG issues (DOM-detectable: missing labels, contrast, ARIA misuse) — keyboard-only and screen-reader passes remain manual, see `context/changes/accessibility-audit-wcag-aa/plan.md` Testing Strategy |
