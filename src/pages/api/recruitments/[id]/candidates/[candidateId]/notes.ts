...
import { upsertCandidateNote } from "@/lib/services/candidates";
...
export const PUT: APIRoute = async (context) => {
  ...
  if (!parsed.success) {
    ...
    return jsonError(422, "invalid_request", "Invalid note data", fields);
  }
  ...
  try {
    const note = await upsertCandidateNote(supabase, parsedId.data, parsedCandidateId.data, parsed.data);
    if (!note) {
