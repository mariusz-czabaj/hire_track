-- S-04: candidate_stage_notes.stage_id references kanban_stages with
-- ON DELETE RESTRICT, so a default stage pinned only by a note would
-- fail update_default_stages' removal with an opaque FK error instead
-- of the documented PA002. Full create or replace, never edit the
-- merged migration -- same guard, third referencing table added.
...
begin
  ...
  -- set. Refuse if any candidate, history row, or note still
  -- references one.
  ...
  -- net additions -- order matters, see 20260901180501's header comment.
